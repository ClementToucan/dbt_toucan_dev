...
SELECT
    ...
    CASE 
      WHEN activity_date IS NOT NULL AND  interactions = 'Stories' THEN 'stories viewed'
      WHEN activity_date IS NOT NULL AND  interactions = 'Homepage' THEN 'homepages viewed'
      WHEN activity_date IS NOT NULL AND  interactions = 'PDF Report' THEN 'PDF reports viewed'
      WHEN activity_date IS NOT NULL AND  interactions = 'Send Home by Mail' THEN 'homepages sent by mail'
      WHEN activity_date IS NOT NULL AND  interactions = 'My Favorites' THEN 'My Favorites viewed'
      WHEN activity_date IS NOT NULL AND  interactions = 'Datawall' THEN 'datawalls displayed'
      WHEN activity_date IS NOT NULL AND  interactions = 'Stories shared by Mail' THEN 'stories shared by Mail'
      WHEN activity_date IS NOT NULL AND  interactions = 'All interactions' THEN 'Interactions'
      ELSE ' Not viewed'
    END AS unit
